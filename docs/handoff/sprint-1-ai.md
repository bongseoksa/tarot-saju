...
- 3라운드 프롬프트 튜닝 수행:
  - 1차: 카드 한국어 이름 필수 사용 + 한줄 요약 필수 포함 → 92%
  - 2차: 각 섹션에서 카드 이름 언급 강제 → 96%
  - 3차: 섹션 헤더 정확히 사용 강제 + 길이 제한 + guard 최대 길이 1500자로 조정 → 100%
- evaluator에 22장 카드 이름 별칭(aliases) 하드코딩 (모델의 이름 변형 허용)
