...
> 완료 기준: 각 페이지가 deep-research-report 레이아웃 템플릿(허브형 홈, 집중형 카드 뽑기, 리포트형 결과)과 일치하는 반응형 레이아웃을 렌더링하고, 모의(mock) 데이터로 정상 동작한다. tsc/lint/test/build 통과.
> **상태: 재작업 필요** — 반응형 레이아웃 + 데스크톱 적응형 패널 구현 필요
