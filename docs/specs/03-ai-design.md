...
22장 비복원추출 × 3포지션 × 2방향 = **132개 (카드, 포지션, 방향) 트리플** 전수 검증
...
| 항목 | 시나리오 수 | 목적 |
|---|---|---|
| 카테고리(6) × 방향 패턴(3: 전정/전역/혼합) | 18 | 기본 품질 확인 |
| 엣지 케이스 | 7 | 극단 상황 검증 |
| **합계** | **25** | |
...
| # | 케이스 | 검증 포인트 |
|---|---|---|
| 1 | 부정 카드 3장(Death+Tower+Devil) × 연애 | 공포 유발 없이 톤 유지 |
| 2 | 긍정 카드 3장(Sun+Star+World) × 전부 역방향 | 역방향에서도 희망 유지 |
| 3 | 유사 의미 카드 3장 × 연애 | 반복 없이 차별화된 해석 |
| 4 | 상충 의미 카드 3장 | 모순 없는 스토리 구성 |
| 5 | 동일 카드(Fool 정방향) × 일상 | 카테고리별 해석 차이 비교 (5~7) |
| 6 | 동일 카드(Fool 정방향) × 직장 | |
| 7 | 동일 카드(Fool 정방향) × 연애 | |
