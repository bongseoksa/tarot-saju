...
- deep-research-report.md — 레이아웃 리서치 + 반응형 개선 제안 (KRDS 그리드, 접근성, KPI)
...
- 디자인: deep-research-report 기반 레이아웃 + KRDS 반응형 그리드 → Tailwind 코드 구현
...
- MVP 도메인: `jeomhana.vercel.app` (Vercel 기본 도메인, 커스텀 도메인은 MVP 검증 후 검토)
...
- **MVP 단계: 개인 (사업자 등록 없이) 진행**
  - Google AdSense는 개인(비사업자)으로 가입 및 수익 수령 가능
  - 수익 300만원 이하: 기타소득 분리과세 (8.8% 원천징수)
  - 외화 수익이므로 5월 종합소득세 신고 필수 (홈택스)
- **사업자 전환 시점**
  - 지속적·반복적 수익이 발생하여 국세청이 사업소득으로 판단할 규모
  - 서버비 등 비용 공제가 필요한 경우
  - 전환 시 간이과세 개인사업자로 시작
...
- **목표**: 100명 유저 확보, 비용 0원
- **채널 우선순위**:
  1. 지인 시딩 — 카톡/인스타 DM 직접 공유 (10~20명)
  2. 에브리타임 — 대학생 타겟, 자유게시판
  3. 인스타그램 — 타로/운세 관심층, 결과 스크린샷
  4. 블라인드 — 직장인 타겟, 사이드프로젝트 소개
  5. 틱톡/숏폼 — 여력 시
- **런칭 순서**: 지인(D-day) → 에브리타임(D+1~3) → 인스타(D+3~7) → 블라인드(D+7~)
- **공유 유도**: 결과 페이지 공유 버튼 + OG 태그 + CTA "점하나에서 타로 봤는데 소름..."
