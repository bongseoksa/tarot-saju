...
- Gemma4 e4b (8B, Q4_K_M) — 로컬 PC에서 Ollama로 서빙
...
- **cloudflared**: Homebrew로 설치 완료 (`brew install cloudflared`)
- **Named Tunnel**: `jeomhana` (Tunnel ID: `3017c817-161c-456d-a91f-2cf657a24754`) — 시스템 데몬 등록 완료
- **현재 방식**: Quick Tunnel (도메인 미보유로 Public Hostname 미설정)
  - `cloudflared tunnel --url http://localhost:11434` 실행 시 `xxxx.trycloudflare.com` URL 자동 생성
  - 실행할 때마다 URL이 바뀜 (도메인 구매 후 Named Tunnel로 전환 예정)
- **Ollama 필수 환경 변수**:
  - `OLLAMA_ORIGINS="*"` — 외부 Origin 허용 (CORS)
  - `OLLAMA_HOST="0.0.0.0:11434"` — 외부 Host 헤더 허용 (터널 경유 시 필수)
- **설정 가이드**: `docs/reference/cloudflare-tunnel-setup.md`

### PC 사양별 가이드

| VRAM | 가능한 모델 | 예상 응답 속도 |
|---|---|---|
| 없음 (CPU only) | gemma2:2b (양자화) | 30초~1분+ |
| 4~6GB | gemma2:2b | 5~15초 |
| 8GB+ | gemma4:e4b (8B) | 30~90초 |
| 16GB+ | gemma4:e4b (8B) | 10~30초 |

> **현재 사용 모델**: `gemma4:e4b` — gemma2:2b 대비 존댓말/심리적 공감 규칙 준수율이 크게 향상됨. 응답 시간이 길어지므로 FE 타임아웃을 90초로 설정.
...
- **자동 실행**: macOS `launchd` plist — cloudflared Named Tunnel 서비스 등록 완료 (로그인 시 자동 시작). Ollama는 수동 실행 필요 (환경 변수 설정 때문)
...
| 변수 | 용도 |
|---|---|
| `OLLAMA_URL` | Ollama 엔드포인트 — Quick Tunnel URL (백엔드 전용, 실행마다 변경) |
